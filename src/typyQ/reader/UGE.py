...
def read_queue(user_group=None):
  if user_group is None:
    qstat_str = subprocess.check_output(shlex.split('qstat -u "tbm" -pri -ext'), text=True)
  else:
    qstat_str = subprocess.check_output(shlex.split('qstat -u "{}" -pri -ext'.format(user_group)), text=True)
  job_nums=[]
  basic_job_data={}
  ...
  job_details = subprocess.check_output(shlex.split('qstat -j {}'.format(job_nums_str)), text=True)
